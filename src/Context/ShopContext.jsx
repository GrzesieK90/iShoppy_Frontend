...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    fetch('http://13.60.170.233:3000/allproducts')
      .then((response) => {
        if (!response.ok) {
    ...
    if (token) {
      fetch('http://13.60.170.233:3000/getcart', {
        method: 'POST',
        headers: {
  ...
  const addToCart = (itemId, size) => {
    ...
    const token = localStorage.getItem('auth-token');
    if (token) {
      fetch('http://13.60.170.233:3000/addtocart', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'auth-token': token,
  ...
  const removeFromCart = (itemId) => {
    ...
    const token = localStorage.getItem('auth-token');
    if (token) {
      fetch('http://13.60.170.233:3000/removefromcart', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'auth-token': token,
